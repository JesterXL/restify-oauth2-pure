...
const setLinkHeader = (res, options, grantTypes) =>
    res.header("Link", `<${options.tokenEndpoint}>; rel="oauth2-token"; grant-types="${grantTypes}"; token-types="bearer"`);

const setWwwAuthenticateHeader = (res, options, error) =>
    res.header("WWW-Authenticate",
        `Bearer realm="${options.wwwAuthenticateRealm}", error="${statusCodesToErrorCodes[error.statusCode]}", error_description="${error.messsage}" `);
